...
//  Author:  Matthias Wölfel, ported to 'sfe' by John McDonough and ABC
//
...
WarpedTwiceMVDRFeature::WarpedTwiceMVDRFeature(const VectorFloatFeatureStreamPtr& src, unsigned order, unsigned correlate, float warp, bool warpFactorFixed, float sensibility, const String& nm)
  : VectorFeatureStream(src->size()/2+1, nm), WarpedTwiceFeature(order, src->size()),
    _src(src), _temp_order(2*order+1),
    _correlate(correlate), _warp(warp),
    _warpFactorFixed(warpFactorFixed), _sensibility(sensibility),
    _tmpR(new float[order+1]), _tmpA(new float[order+1]),
    _tmpPC(new float[_temp_order]), _tmpPA(new float[WarpedTwiceFeature::_dim+1]),
...
const gsl_vector* WarpedTwiceMVDRFeature::next(int frameX)
{
  ...
  unsigned log2N = (unsigned) ceil( log((double)WarpedTwiceFeature::_dim) / log((double)2.0) );
...
const gsl_vector* SpectralSmoothing::next(int frameX)
{
  if (frameX == _frameX) return _vector;
  ...
  const gsl_vector* _maxSpec = _adjustTo->next(_frameX + 1);
  const gsl_vector* _maxFFT  = _adjustFrom->next(_frameX + 1);
  _increment();
  ...
  for (unsigned i = 0; i < size(); i++)
    gsl_vector_set(_vector, i, mult * gsl_vector_get(_maxSpec, i));
