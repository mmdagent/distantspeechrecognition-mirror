...
const gsl_vector_float* LPCSpectrumEstimator::next(int frameX)
{
  ...
  const gsl_vector_float* samples = _samp->next(_frameX+1);
  calcLPCs( samples );
  calcLPEnvelope();

  /*
  static char fileName[256];
  sprintf(fileName, "spectra/frame%04d.txt", frameX);
  FILE* fp = fileOpen(fileName, "w");
  for (unsigned i = 0; i <= _fftLen / 2; i++)
    fprintf(fp, "%12.4f   %12.4f\n", 16000.0 * i / _fftLen, 10*log10(gsl_vector_float_get(_vector, i)));
...
{
  gsl_vector_float*  lpEnvelope = _vector;
...
const gsl_vector_float* CepstralSpectrumEstimator::next(int frameX)
{
  static char fileName[256];
  ...
  /*
  sprintf(fileName, "fft/%s-%04d.txt", name().c_str(), frameX);
  FILE* fp = fileOpen(fileName, "w");
  for (unsigned i = 0; i <= _fftLen / 2; i++)
    fprintf(fp, "%12.4f   %12.4f\n", 16000.0 * i / _fftLen, 10*log10(gsl_complex_abs2(gsl_vector_complex_get(block, i))));
  fileClose(fileName, fp);
  */
  ...
  halfComplexPack(_samples, _cepstrum, _fftLen);
  ...
  /*
  sprintf(fileName, "cepstra/%s-%04d.txt", name().c_str(), frameX);
  ...
  */
  ...
  halfComplexUnpack(_spectrum, _samples);

  // exponentiate log-spectrum to obtain the cepstral spectral envelope
  for (unsigned i = 0; i < _fftLen; i++)
    gsl_vector_float_set(_vector, i, exp(gsl_complex_abs(gsl_vector_complex_get(_spectrum, i))));

  /*
  sprintf(fileName, "spectra/%s-%04d.txt", name().c_str(), frameX);
  fp = fileOpen(fileName, "w");
  for (unsigned i = 0; i <= _fftLen / 2; i++)
    fprintf(fp, "%12.4f   %12.4f\n", 16000.0 * i / _fftLen, 10*log10(gsl_vector_float_get(_vector, i)));
  fileClose(fileName, fp);
