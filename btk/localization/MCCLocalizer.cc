...
void SGB4LinearArray::setPositionsOfMicrophones( const gsl_matrix* mpos )
{
  size_t nMic = _mpos->size1;
  if( nMic != mpos->size1 ){
    printf("The size of the matrix for the geometry of the array should be %d x %d\n", nMic, _mpos->size2 );
    throw jdimension_error("The size of the matrix for the geometry of the array should be %d x %d\n", nMic, _mpos->size2 );
  }
...
{ 
  ...
  if( _blockList[0]->size < 2 * maxSampleDelay ){
    fprintf(stderr,"Data samples are insufficient. It must be more than\n", 2 * maxSampleDelay );
  ...
  for (int frameX=frameS;frameX<frameN;frameX++,nSamples++){
    for(size_t chanX=0;chanX<chanN;chanX++){
      int frameY = frameX + _tau[chanX];
      float val;
      if( frameY >= 0 ){
...
gsl_vector* MCCLocalizer::search( int frameX )
{
...
#define TAKE_MAX_IN_ONE_UTT
  ...
  //_minMCCC = 1 - minCostV;
  gsl_vector_memcpy(_vector, _sourceCandidates[0]->_position);

  return _vector;
}

const gsl_vector* MCCLocalizer::next(int frameX )
{
  setIncomingData( frameX );
  _vector = search( frameX );
  return _vector;
}
...
const gsl_vector* MCCCalculator::next(int frameX )
{
  double costV;
  //_sgbPtr->reset();
  //const gsl_vector *position = _sgbPtr->getSearchPosition();
  if( NULL == _delays ){
    fprintf(stderr,"set time delays with setTimeDelays()\n");
  ...
  gsl_vector_set(_vector, 0, costV );
  _sourceCandidates[0]->setSourceInfo( _tau, _vector, costV, _eigenvalues );
...
{
  double costV = gsl_vector_get(_vector,0);
#ifdef _NO_LOG_
  return ( 1 - costV );
#else
...
double MCCCalculator::getCostV(){ 
  return gsl_vector_get(_vector, 0);
}
...
RMCCLocalizer::RMCCLocalizer( SearchGridBuilderPtr &sgbPtr, float lambda, size_t maxSource, const String&nm )
  :MCCLocalizer( sgbPtr, maxSource, nm ),_lambda(lambda)
{
  size_t chanN = sgbPtr->chanN();
...
const gsl_vector* RMCCLocalizer::next(int frameX )
{
  return _vector;
}
