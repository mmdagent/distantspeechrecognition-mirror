...
/**
   @brief In order to do efficient grid-search for the source position which provides the maximum/minimum objective function, each cell size of a search grid has to be "reasonable". This class constructs such a search grid and return the time delay corresponding to each position on the grid.
   @usage
...
private:
  ...
  const float _lambda; /* a forgetting factor */
};
