...
static gsl_matrix_float* _gsl_matrix_float_load(gsl_matrix_float* m, const char* fileName)
{
  int ret = 0;
  FILE* mf = fileOpen(fileName, "r");
  if (gsl_matrix_float_fread(mf, m) < 0) {
    fclose(mf);
    throw jio_error("Could not read matrix from %s", fileName);
  }

  fileClose(fileName, mf);
  return m;
}

static gsl_matrix_float* _gsl_matrix_float_load_old(gsl_matrix_float* m, const char* filename)
{
  int pos, left, total;
  int size1, size2;
  long total_size;
...
{
  if (old)
    return _gsl_matrix_float_load_old(m, filename);

  return _gsl_matrix_float_load(m, filename);
}
