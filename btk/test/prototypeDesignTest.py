...
PROTOTYPE_DIR = './prototype.pickle'

M      = 16
m      = 8

#-- if M,m are passed by command line, use those
if (len(sys.argv) == 3):
    try:
        M_test = int(sys.argv[1])
        m_test = int(sys.argv[2])        
    except ValueError: pass
    else:
        M = M_test
        m = m_test
        print "creating new prototype with M=%d, m=%d"%(M,m)
...
if (not os.path.exists(PROTOTYPE_DIR)): os.mkdir(PROTOTYPE_DIR)
prototypeFileName = '%s/prototype_M%d_m%d.pickle'%(PROTOTYPE_DIR, M,m)
